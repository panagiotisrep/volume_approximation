...
//' @export
// [[Rcpp::export]]
Rcpp::NumericMatrix sample_points_sparse(Rcpp::Nullable<Rcpp::CharacterVector> file = R_NilValue,
                                  Rcpp::Nullable<std::string> distribution = R_NilValue,
                                  Rcpp::Nullable<unsigned int> N = R_NilValue,
                                  Rcpp::Nullable<unsigned int> walk_length = R_NilValue,
                                  Rcpp::Nullable<double> Temperature = R_NilValue,
                                  Rcpp::Nullable<unsigned int> random_walk = R_NilValue,
                                  Rcpp::Nullable<Rcpp::CharacterVector> initial_point_file = R_NilValue){
    ...
    lmi Slmi;
    VT c;
    loadSparseSDPAFormatFile3<MT>(inp, Slmi, c);
    unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
    // the random engine with this seed
    ...
    // if no file with an initial point was given, take the origin as initial point
    // otherwise read the file
    Point p(n);

    if (initial_point_file.isNotNull()) {
        // open stream
        std::ifstream inpF;
        inpF.open(Rcpp::as<std::string>(initial_point_file),std::ifstream::in);
...
//    SP.ComputeInnerBall(diam, radius);
    radius=diam=0.4; //TODO chdr not working
    std::pair<Point,NT> InnerB;
    InnerB.first = p;
